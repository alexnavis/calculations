...
describe('integration of output evaluators', function () {
  ...
  describe('generation of an array of evaluators', function () {
    it('should be able to generate a single evaluator with a callback', done => {
      GENERATE({ segments: [ MOCKS.DEFAULT, MOCKS.MULTIPLE ] }, (err, evaluators) => {
        try {
          if (err) throw err;
          else {
            expect(evaluators).to.have.property('new_calc');
            expect(evaluators).to.have.property('new_calc_1');
            done();
          }
        } catch (e) {
          done(e);
    ...
    it('should be able to generate a single evaluator and return a Promise', done => {
      GENERATE({ segments: [ MOCKS.DEFAULT, MOCKS.MULTIPLE ] })
        .try(evaluators => {
          expect(evaluators).to.have.property('new_calc');
          expect(evaluators).to.have.property('new_calc_1');
          done();
